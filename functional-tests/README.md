# ParsePort functional tests

Dutch sentence parser for Spindle + Æthel (and maybe others in the future).

This is a functional test suite based on [pytest][1] and [Selenium WebDriver][2]. It is designed such that it can test any network-reachable instance of the ParsePort application. While the tests are based on knowledge about the user interface of the application, no knowledge of its implementation is needed.
...
Every test in this suite is a short, automated simulation of the interaction between a user and ParsePort, written in Python. [Selenium][2] provides the programmable web browser interface while [pytest][1] provides administrative support in the form of configuration and fixtures.
...
```console
$ pytest --base-address http://localhost:5001/
$ pytest --base-address https://www.parseport.com/app/
```
...
webdriver =
    Safari
addopts = --base-address http://localhost:5001/
```
